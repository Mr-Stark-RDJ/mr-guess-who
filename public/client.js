...
let heroes=[],flipped=new Set(),joinedRoom="",socket=null,secret=null,nonce=null;
let secretMode=false,committed=false,allHidden=false,myId=null;
...
function showNextAlert(){const n=alertQueue.shift();if(n){alertBody.textContent=n;alertModal.classList.remove("hidden");alertOpen=true}else{alertModal.classList.add("hidden");alertOpen=false}}
...
function render(){
  ...
  board.querySelectorAll(".card").forEach(c=>{
    ...
    c.onclick=()=>{if(secretMode){if(committed){pushAlert("Secret is locked.");return}selectSecret(id)}else{toggle(id)}};
  });
  drawSecret();
}
