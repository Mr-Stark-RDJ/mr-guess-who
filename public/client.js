...
alertModal.addEventListener("click", (e) => {
  if (e.target === alertModal) showNextAlert();
});
...
const randHex = (len = 16) => {
  ...
  return Array.from(a).map(tenc).join("");
};
async function sha256Hex(s) {
  const d = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(s));
  return Array.from(new Uint8Array(d)).map(tenc).join("");
}
...
function drawSecret() {
  if (!secret) {
    secretSlot.textContent = "Not selected";
    lockSecretBtn.disabled = true;
    return;
  }
  const h = heroes.find((x) => x.id === secret);
  secretSlot.innerHTML = `<div class="card">
...
function ensureSocket() {
  ...
  socket.on("reveal:request", () => {
    if (committed && secret && nonce) {
      socket.emit("secret:reveal", { room: joinedRoom, secret, nonce });
    }
  });
...
function join() {
  ...
  ensureSocket();
  joinedRoom = r;
  socket.emit("join", { code: r, pid: myPid, name: myName || "Player" });
...
async function lockSecret() {
  if (!joinedRoom) {
    pushAlert("Join a room first.");
    return;
  }
  if (!secret) {
    pushAlert("Pick a secret in Secret Mode.");
    return;
  }
  nonce = randHex(16);
...
function submitGuess() {
  ...
  if (onCount !== 1) {
    pushAlert("Keep only one hero visible to submit your guess.");
    return;
  }
  const id = onlyOn();
  if (!id || !joinedRoom) {
...
if (!localStorage.getItem("rivals_pid")) {
  localStorage.setItem("rivals_pid", (crypto?.randomUUID?.() ?? randHex(16)).replace(/-/g, ""));
}
myPid = localStorage.getItem("rivals_pid");
